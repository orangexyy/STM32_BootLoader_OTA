...
void bootloader_enter_info_printf(void)
{
    ...
    usart3_printf("[3]Download The Program To External FLASH\r\n");
    usart3_printf("[4]Download The Program From External FLASH\r\n");
    usart3_printf("[5]Get OTA Version\r\n");
    usart3_printf("[6]Get Information Of The Program In External FLASH\r\n");
    usart3_printf("[7]Get Program From Internet And Download To External FLASH\r\n");
...
void bootloader_ota_a_block(void)
{
    ...
    memcpy(ota_info_struct.version[0], ota_info_struct.version[update_a_struct.data_block_num], 8);
    at24c256_write_ota_data();
    
    usart3_printf("Size Of The Bin File In Block %d : %d  version : %-6s\r\n",update_a_struct.data_block_num, ota_info_struct.app_data_size[update_a_struct.data_block_num], ota_info_struct.version[update_a_struct.data_block_num]);
...
void bootloader_erase_a_block(void)
{
    flash_erase(FLASH_BLOCK_A_START_PAGE,FLASH_BLOCK_A_PAGE_NUM);
    at24c256_read_ota_data();
    memset(ota_info_struct.version[0], 0, 8);
    at24c256_write_ota_data();
...
void bootloader_iap_start(void)
{
    ...
    else if(xmodem_protocol_struct.direction_flag == 2)
    {
        ...
        xmodem_protocol_struct.time = 0;
        xmodem_protocol_struct.receive_buf_num = 0;
        xmodem_protocol_struct.server_connect_flag = 0;
        ota_info_struct.app_data_size[update_a_struct.data_block_num] = 0;
        memset(ota_info_struct.version[update_a_struct.data_block_num], 0, 8); 
        w25q64_sector_erase_64k(update_a_struct.data_block_num);
        at24c256_write_ota_data();          //擦除之后写入，查询时才会更新 
...
void bootloader_iap_ready(void)
{
    ...
    if(xmodem_protocol_struct.time >= 100)
    {
        ...
        {
            if (xmodem_protocol_struct.server_connect_flag == 0)
            {
...
void bootloader_iap_receive(void)
{
    if (xmodem_protocol_struct.direction_flag == 2)
    {
        ...
        if(xmodem_protocol_struct.receive_crc == usart2_rx_buffer[131]*256 + usart2_rx_buffer[132])       //crc检验
        {
            ...
            {
                print_counter = 0;
                memcpy(ota_info_struct.version[update_a_struct.data_block_num], &usart2_rx_buffer[133], 8); 
                // usart3_printf("version : %s\r\n", ota_info_struct.version[update_a_struct.data_block_num]);
                //总字节大小、已接收字节大小、进度
                ...
                usart3_printf("progress : %d%% (%u/%u Byte)  version : %s\r\n", progress, xmodem_protocol_struct.received_size, xmodem_protocol_struct.total_size, ota_info_struct.version[update_a_struct.data_block_num]);
                if(progress >= 100)
                {
                    progress = 0;
                }
...
uint8_t bootloader_select_flash_block(void)
{
    ...
    if(usart3_rx_buffer_len == 6)
    {
        ...
        else
        {
            ...
            usart3_printf("Select the FLASH Block(1~7) Again:(Format:BLOCKx)\r\n");
...
void bootloader_get_info_of_external_flash(void)
{
    ...
    for (i=1; i<8; i++)
    {
        usart3_printf("app_data_size[%d]: %6d | version : %-6s\r\n", i, ota_info_struct.app_data_size[i], ota_info_struct.version[i]); // 调试打印
    }
    delay_ms(1000);
    bootloader_enter_info_printf();
}
...
{   
    ...
    if(get_usart3_receive_flag())
    {
        ...
        else if((usart3_rx_buffer_len == 1) && (usart3_rx_buffer[0] == '3'))
        {
            ...
            usart3_printf("Select the FLASH Block(1~7):(Format:BLOCKx)\r\n");
        ...
        {
            usart3_printf("[6]Get Information Of The Program In External FLASH\r\n");
            bootloader_current_event = BOOTLOADER_EVENT_GET_INFO_OF_EXTERNAL_FLASH;
        }
    ...
    if(get_usart2_receive_flag())
    {
        ...
        else if((usart2_rx_len == 6) && (usart2_rx_buffer[4] == 0x04))
        {
            xmodem_protocol_struct.server_connect_flag = 1;
        }
    }
...
{ 
    ...
    switch(bootloader_current_event)
    {
        ...
        case BOOTLOADER_EVENT_IAP_RECEIVE_DETECT: 
            if(bootloader_iap_receive_timeout_detect() == 1)
            {
        ...
        case BOOTLOADER_EVENT_DOWNLOAD_FROM_EXTERNAL_FLASH: 
            ...
            break;
        case BOOTLOADER_EVENT_GET_INFO_OF_EXTERNAL_FLASH:
            bootloader_get_info_of_external_flash();
            bootloader_current_event = BOOTLOADER_EVENT_NONE;		//清除标志位 
            break;
