...
typedef struct 
{
    ...
    uint32_t    app_data_size[15];           //应用程序所占的内存大小，第0个代表OTA程序，后面的为存放在外部flash的应用程序
    uint8_t     version[8][8];              //V1.0.0  (6个字节)    应用程序的版本号，第0个代表OTA程序，后面的为存放在外部flash的应用程序
} OTA_INFO_DATA;

typedef struct 
{
    uint8_t buffer[FLASH_PAGE_SIZE];
    uint8_t data_block_num;            //应用程序存放在外部flash的扇区位置，0为OTA程序放在0号block，后面的为存放在外部flash的应用程序
