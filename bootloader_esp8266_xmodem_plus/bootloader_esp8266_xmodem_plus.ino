...
// Xmodem配置（新增帧长定义）
#define XMODEM_FRAME_EXTENDED_SIZE (XMODEM_BLOCK_SIZE + 5 + 8)  // 128+5(原有)+8(新增)=141字节
...
bool sendXmodemBlock(const uint8_t *data, size_t length) {
    ...
    // 计算CRC（原有部分不变）
    uint16_t crc = crc16(&buffer[3], XMODEM_BLOCK_SIZE);
    buffer[3 + XMODEM_BLOCK_SIZE] = (crc >> 8) & 0xFF;  // CRC高字节（位置131）
    buffer[4 + XMODEM_BLOCK_SIZE] = crc & 0xFF;          // CRC低字节（位置132）
    
    // 新增：在CRC后追加总大小和已发送大小（各4字节，大端序）
    uint32_t sentSize = downloadOffset + length;  // 已发送的总字节数（当前块发送后）
    // 总大小（totalSize）拆分为4字节（位置133-136）
    buffer[133] = (totalSize >> 24) & 0xFF;  // 总大小高位
    buffer[134] = (totalSize >> 16) & 0xFF;
    buffer[135] = (totalSize >> 8) & 0xFF;
    buffer[136] = totalSize & 0xFF;          // 总大小低位
    // 已发送大小（sentSize）拆分为4字节（位置137-140）
    buffer[137] = (sentSize >> 24) & 0xFF;   // 已发送大小高位
    buffer[138] = (sentSize >> 16) & 0xFF;
    buffer[139] = (sentSize >> 8) & 0xFF;
    buffer[140] = sentSize & 0xFF;           // 已发送大小低位
    
    // 发送扩展帧（141字节：原有133字节 + 新增8字节）
    Serial.write(buffer, XMODEM_FRAME_EXTENDED_SIZE);
